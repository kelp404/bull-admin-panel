  ...
  listenQueuesEvents() {
    this.queues.forEach(queue => {
      queue.on('global:waiting', jobId => {
        this.jobEventHandler(EventType.WAITING, queue.name, jobId);
      });
      queue.on('global:active', jobId => {
        this.jobEventHandler(EventType.ACTIVE, queue.name, jobId);
      });
      queue.on('global:completed', jobId => {
        this.jobEventHandler(EventType.COMPLETED, queue.name, jobId);
      });
      queue.on('global:failed', jobId => {
        this.jobEventHandler(EventType.FAILED, queue.name, jobId);
      });
      queue.on('global:paused', jobId => {
        this.jobEventHandler(EventType.PAUSED, queue.name, jobId);
      });
      queue.on('removed', job => {
        ...
        this.jobEventHandler(EventType.REMOVED, queue.name, job.id);
      });
    });
  }
