...
module.exports = class AdminPanel {
   ...
   */
  constructor({expressRouter, basePath, verifyClient, queues, server}) {
  ...
  onConnection(ws) {
    ...
    ws.on('message', message => {
      ...
      try {
        ...
        response = new Response({requestId: request.id, ws: ws});
        const result = this.dispatcher(request, response, () => {
   ...
   */
  jobEventHandler(eventType, queueName, jobId) {
    Object.keys(this.pool).forEach(wsId => {
      const notification = new Notification({ws: this.pool[wsId]});
      notification.json(eventType, {queueName, id: jobId});
    });
  }
  ...
  listenQueuesEvents() {
    this.queues.forEach(queue => {
      queue.on('global:waiting', jobId => {
        this.jobEventHandler(Notification.eventTypes.JOB_WAITING, queue.name, jobId);
      });
      queue.on('global:active', jobId => {
        this.jobEventHandler(Notification.eventTypes.JOB_ACTIVE, queue.name, jobId);
      });
      queue.on('global:completed', jobId => {
        this.jobEventHandler(Notification.eventTypes.JOB_COMPLETED, queue.name, jobId);
      });
      queue.on('global:failed', jobId => {
        this.jobEventHandler(Notification.eventTypes.JOB_FAILED, queue.name, jobId);
      });
      queue.on('global:paused', jobId => {
        this.jobEventHandler(Notification.eventTypes.JOB_PAUSED, queue.name, jobId);
      });
      queue.on('removed', job => {
        ...
        this.jobEventHandler(Notification.eventTypes.JOB_REMOVED, queue.name, job.id);
      });
    });
  }
