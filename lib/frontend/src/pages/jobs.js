...
module.exports = class Jobs extends Base {
  ...
  constructor(props) {
    ...
    this.state.rowHeight = props.params.state === jobStates.FAILED ? 106 : 81;
    this.listRef = null;
    ...
    const jobNotificationHandler = job => {
      ...
      @param job {JobModel}
       */
      if (job.state === (props.params.state || jobStates.ACTIVE)) {
    ...
    this.$listens.push(
      api.subscribe(eventTypes.JOB_ENQUEUE, job => {
        jobNotificationHandler(job);
      })
    );
    this.$listens.push(
      api.subscribe(eventTypes.JOB_START, job => {
        jobNotificationHandler(job);
      })
    );
    this.$listens.push(
      api.subscribe(eventTypes.JOB_COMPLETE, job => {
        jobNotificationHandler(job);
      })
    );
    this.$listens.push(
      api.subscribe(eventTypes.JOB_FAILED, job => {
        jobNotificationHandler(job);
      })
    );
    this.$listens.push(
      api.subscribe(eventTypes.JOB_REMOVE, job => {
        if (this.removeJob(job)) {
          // Decrease the job quantity at the navigation.
          const jobQuantity = store.get('$jobQuantity');
          jobQuantity[(props.params.state || jobStates.ACTIVE)] -= 1;
  ...
  rowRenderer({index, key, style}) {
    ...
    if (this.props.params.state === jobStates.FAILED && job.error) {
      shortErrorMessage = `${job.error}`.match(/^(.*)\n?/)[1];
    }

    return (
      <div
        key={key} className={className} style={{
          ...style,
