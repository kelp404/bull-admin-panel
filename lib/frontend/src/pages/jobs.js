...
module.exports = class Jobs extends Base {
  ...
  constructor(props) {
    ...
    this.$listens.push(
      api.subscribe(api.eventTypes.JOB_ENQUEUE, job => {
        if (props.params.state === 'inactive') {
          // Push the new job into the grid.
          const jobIndex = this.state.jobs.findIndex(x => x.id === job.id);
          if (jobIndex >= 0) {
            // The job is already in the grid.
            this.updateJob(jobIndex, job);
          } else if (props.params.sort === 'asc') {
            this.insertJob(job, 'bottom');
          } else {
            this.insertJob(job, 'top');
          }
        }
      })
    );
  ...
  insertJob(job, position) {
    ...
    this.setState(prevState => {
      ...
      if (position === 'bottom') {
        nextState.jobs.push(job);
      } else {
        nextState.jobs.unshift(job);
      }
  ...
  removeJob(index) {
    ...
    this.setState(prevState => {
      const nextState = Object.assign({}, prevState);
      nextState.jobs[index].animateDelete = true;
      return nextState;
    });
    this.listRef.forceUpdateGrid();
    setTimeout(() => {
      ...
      api.job.restartJob(job.id)
        .then(result => {
          ...
          if (result.state !== job.state) {
            // The job was changed the state.
            // We should remove it from the grid.
            this.removeJob(index);
          }
        })
        .catch(error => {
