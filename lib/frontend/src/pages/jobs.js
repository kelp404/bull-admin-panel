...
module.exports = class Jobs extends Base {
  ...
  constructor(props) {
    ...
    this.$listens.push(
      api.subscribe(eventTypes.JOB_PAUSED, data => {
        this.notificationHandler(eventTypes.JOB_PAUSED, data.queueName, data.id);
      })
    );
    this.$listens.push(
    ...
    api.job.getJob(queueName, jobId)
      .then(job => this.setState(prevState => {
        ...
        const jobEventStateMapping = {
          [eventTypes.JOB_WAITING]: jobStates.WAITING,
          [eventTypes.JOB_ACTIVE]: jobStates.ACTIVE,
          [eventTypes.JOB_COMPLETED]: jobStates.COMPLETED,
          [eventTypes.JOB_FAILED]: jobStates.FAILED,
          [eventTypes.JOB_PAUSED]: jobStates.PAUSED
        };
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <div>
                        <OverlayTrigger overlay={<Tooltip>Remove</Tooltip>}>
                          <a className="btn btn-link text-danger" href="#delete" onClick={this.generateClickRemoveJobLinkHandler(job.id)}>
