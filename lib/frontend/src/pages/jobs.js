...
module.exports = class Jobs extends Base {
  ...
  removeJob(index) {
    /*
    Remove the job in the WindowScroller with the animation.
    @param index {Number}
     */
    this.setState(prevState => {
      const result = Object.assign({}, prevState);
      result.jobs[index].isDeleting = true;
      return result;
    });
  ...
  generateDeleteJobHandler(index) {
    ...
    return () => {
      ...
      nprogress.start();
      api.job.deleteJob(this.state.jobs[index].id)
        .catch(error => {
  ...
  rowRenderer({index, key, style}) {
    ...
    const className = classNames([
      'list-group-item',
      {'animated faster zoomOutDown': job.isDeleting}
    ]);
